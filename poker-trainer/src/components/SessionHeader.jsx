 ...
 */
export function SessionHeader({ stats, streak, bestStreak, format, compact = false, onReset }) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-background/85 backdrop-blur-lg">
      <div className="mx-auto w-full max-w-3xl px-3 pb-2.5 pt-3 sm:px-4">
        <div className="mb-2 flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-[10px] text-muted-foreground">
              {formatName} · exploitative decisions
            </p>
          </div>
          {compact && stats.total > 0 ? (
            <span className={cn('tabular shrink-0 text-sm font-extrabold', tone.text)}>
              {stats.accuracy}%
            </span>
          ) : null}
          <Button
        ...
        {/* Outside the drill the metric grid is noise — collapse to the title row. */}
        {compact ? null : (
          <>
